...
def build_table_row(row_obj,table_obj,onclick_column=None,target_link=None):
    ...
    for index,column_name in enumerate(table_obj.list_display):
        ...
        if onclick_column == column_name:
            ...
            column2 = ''' <td><a href=%s> ''' % (url_reverse(target_link, args=(column_data,)))
            print("--->custom_tag_column2: ",column2)
        else:
            if column_name == 'name':
                column = "%s%s</a></td>" % (column2,column_data)
            else:
                column = "<td>%s</td>" % column_data
            #column = "<td>%s</td>" % column_data
        print('--->custom_tag_row_ele: ', row_ele)
        row_ele +=column
