...
def get_orderby(request, model_objs, admin_form):
    ...
    print("--->assets.tables.get_orderby-orderby_field:",orderby_field)
    if orderby_field:
        print("--->if orderby_field: ",orderby_field)
        ...
        print("--->if orderby_field : error! ")
        print("--->tables.get_orderby-model_objs :  ", model_objs)
        return [model_objs, orderby_field, None]

# 获取 models.Asset 中的资产信息
# model_class = models.Asset 类
# admin_class = admin.AssetAdmin 类
# query_sets = 传递分页信息
# order_res = 排列后的asset_obj_list 数据
class TableHandler(object):
    ...
    # 根据 list_filter 获取设备信息
    def get_list_filter(self, list_filter):
        # This "list_filter" is admin.AssetAdmin.list_filter
        # Get the verbose_name of list_filter from models.Asset
        # ('asset_type', 'status', 'manufactory', 'idc', 'business_unit', 'admin', 'trade_date')
        print("--->tables.TableHandler.get_list_filter-list filters: ",list_filter)

        filters = []
        # 获取 models.Asset 表字段的 verbose_name （在这里是中文名称）
        ...
        # 这个filters就是页面的全部数据
        print("--->tables.TableHandler.get_list_filter-filters: ", filters)
        return filters
...
def table_filter(request, model_admin, models_class):
    ...
    for condition in model_admin.list_filter:
        print("--->table_filter--->request.GET.get(condition): ",request.GET.get(condition))
